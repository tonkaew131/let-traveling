...
function extractAllTripData(messages: UIMessage[]) {
  const flights: any[] = []
  const hotels: any[] = []
  ...
  return { flights, hotels, dayPlans: dayPlans.sort((a, b) => a.day - b.day), summaries }
}

export async function exportTripPDF(messages: UIMessage[]) {
  const { flights, hotels, dayPlans, summaries } = extractAllTripData(messages)
...
<body>
  ...
  <div class="footer">
    Generated by Voyager AI Travel Planner · ${new Date().toLocaleDateString()}
  </div>
</body>
</html>`
